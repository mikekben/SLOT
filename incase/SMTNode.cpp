...
namespace SLOT
{
    ...
    MetadataAsValue* FloatingNode::LLVMRoundingMode()
    {
       switch (RoundingMode())
       {
            case RNE:
                return args.push_back(MetadataAsValue::get(lcx, MDString::get(lcx, "round.tonearest")));
            case RNA:
                return args.push_back(MetadataAsValue::get(lcx, MDString::get(lcx, "round.tonearestaway")));
            case RTP:
                return args.push_back(MetadataAsValue::get(lcx, MDString::get(lcx, "round.upward")));
            case RTN:
                return args.push_back(MetadataAsValue::get(lcx, MDString::get(lcx, "round.downward")));
            case RTZ:
                return args.push_back(MetadataAsValue::get(lcx, MDString::get(lcx, "round.towardzero")));
            default:
                throw UnsupportedSMTOpException("floating point rounding mode", contents);
       }
    }
    MetadataAsValue* FloatingNode::LLVMException()
    {
    ...
    FloatingNode::FloatingNode(LLVMContext& t_lcx, Module* t_lmodule, IRBuilder<>& t_builder, const LLMAPPING& t_variables, expr t_contents) : SMTNode(t_lcx, t_lmodule, t_builder, t_variables, t_contents)
    {
        ...
        /*
        if (contents.num_args() > 0 && contents.arg(0).get_sort().sort_kind() == Z3_ROUNDING_MODE_SORT && contents.arg(0).decl().decl_kind() != Z3_OP_FPA_RM_NEAREST_TIES_TO_EVEN)
        {
            throw UnsupportedSMTOpException("floating point rounding mode", contents);
        }
        */
            ...
            switch (Op())
            {
                ...
                case Z3_OP_FPA_TO_FP_UNSIGNED:
                    ...
                    else
                    {
                        ...
                        fun = Intrinsic::getDeclaration(lmodule, Intrinsic::experimental_constrained_uitofp, FloatingType());
                        return builder.CreateCall(fun, args);
                    }
                case Z3_OP_FPA_TO_FP:
                    ...
                    else if (contents.arg(1).is_bv())
                    {
                        ...
                        else
                        {
                            ...
                            args.push_back(LLVMRoundingMode());
                            args.push_back(LLVMException());
                            fun = Intrinsic::getDeclaration(lmodule, Intrinsic::experimental_constrained_sitofp, FloatingType());
                            return builder.CreateCall(fun, args);
                        }
                    }
                ...
                case Z3_OP_FPA_ROUND_TO_INTEGRAL:
                    ...
                    if (IsRNE())
                    {
                        fun = Intrinsic::getDeclaration(lmodule, Intrinsic::round, FloatingType());
                    }
                    else
                    {
                        args.push_back(LLVMRoundingMode());
                        args.push_back(LLVMException());
                        fun = Intrinsic::getDeclaration(lmodule, Intrinsic::experimental_constrained_round, FloatingType());
                    }
